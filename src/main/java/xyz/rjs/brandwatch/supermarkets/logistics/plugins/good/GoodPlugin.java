 ...
 * the forseeable future. It does not make decisions based on the price. This is
 * reasonable because the price always has an even chance of going up or down
 * (except when it is at rock bottom, when it has an increased chance of staying
 * the same).
 * 
 * NOTE: Review this. The sale price is 10, so if the buy price meets or exceeds
 * that no purchases should be made.
 * 
 * NOTE: The balance is declared on the first tick. However the Balance object
 * has no accessor so it is unsporting to read it. The string form could be
 * easily parsed.
 * 
...
public class GoodPlugin extends AbstractPlugin {
	...
	public void tickListener(ClockTick tick) {
		...
		logger.info(String.format(
			"\nTICK: Delivery Time (%.2f) Purchase per Tick (%.2f) Current Stock (%d) Total Stock (%d) Required Stock (%d)",
			deliveries.confidence(),
			purchases.confidence(this.tick),
			warehouse.getStock(),
			getTotalStock(),
			getRequiredStock()
		));
	...
	private int getTotalStock() {
		return warehouse.getStock() + orders.stream().mapToInt(order -> order.getVolume()).sum();
	}
	...
	/**
	 * Provides the behaviours associated with the different states the GoodPlugin can be in.
	 * 
	 * @author matthew
	 */
	private enum STATE {
		/**
		 * The START state waits for the price to become available. Once the price is available trading can begin.
		 */
		START() {
		...
		GATHER_DATA() {
			...
			void tickListener(GoodPlugin plugin, ClockTick tick) {
				if (plugin.startingTradesIndex < STARTING_TRADES.length) {
